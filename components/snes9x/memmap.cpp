...
{
	if (onlyNonSavedSRAM)
		if (!(ROMType < 0x15) && !(false && ROMType == 0x34)) // can have SRAM
			return;
...
void CMemory::map_System (void)
{
	...
	map_space(0x00, 0x3f, 0x0000, 0x0fff, RAM1);
	map_space(0x00, 0x3f, 0x1000, 0x1fff, RAM2);
	...
	map_space(0x80, 0xbf, 0x0000, 0x0fff, RAM1);
	map_space(0x80, 0xbf, 0x1000, 0x1fff, RAM2);
...
void CMemory::map_WRAM (void)
{
	...
	map_space(0x7e, 0x7e, 0x0000, 0xffff, RAM1);
	map_space(0x7f, 0x7f, 0x0000, 0xffff, RAM2);
}
